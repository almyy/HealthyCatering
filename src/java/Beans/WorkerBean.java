...
public class WorkerBean implements Serializable {
    ...
    private void quickSortDate(int low, int high) {
        int i = low;
        int j = high;
        Date pivot = tabledata.get(low + (high - low) / 2).getOrder().getDate();
        while (i <= j) {
            while (tabledata.get(i).getOrder().getDate().before(pivot)) {
                i++;
            }
            while (tabledata.get(j).getOrder().getDate().after(pivot)) {
                j--;
            }
            if (i <= j) {
                exchange(i, j);i++;j--;
